...
export interface IQuestionTypeModalData {
  title: string;
  collectionKey: string;
  item: IQuestionTypeItem | null;
  order: number;
}

export interface IQuestionTypeModalResult {
  saved: boolean;
  mode: 'create' | 'edit';
  payload: {
    name: string;
...
@Component({
  ...
  styleUrl: './question-type-modal.component.scss',
})
export class QuestionTypeModalComponent {
  readonly data = input.required<IQuestionTypeModalData>();
  ...
  protected readonly form = this.fb.nonNullable.group({
    ...
    show_instructions: [false, [Validators.required]],
    show_question: [true, [Validators.required]],
  ...
  protected onSubmit(): void {
    ...
    this.modal.close<IQuestionTypeModalResult>({
      saved: true,
  ...
  private buildPayload(): IQuestionTypeModalResult['payload'] {
    const value = this.form.getRawValue();

    return {
      name: value.name.trim(),
      translation: value.translation.trim() || null,
      show_instructions: value.show_instructions,
      show_question: value.show_question,
      active: value.active,
