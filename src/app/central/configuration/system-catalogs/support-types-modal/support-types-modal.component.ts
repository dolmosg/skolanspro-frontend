...
export interface ISupportTypeModalData {
  id?: number;
  name?: string;
  translation?: string;
}

export interface ISupportTypeModalResult {
  saved: boolean;
  mode: 'create' | 'edit';
  payload: {
    name: string;
...
@Component({
  selector: 'app-support-types-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TranslateModule, UiButtonComponent, FormErrorComponent],
  templateUrl: './support-types-modal.component.html',
  styleUrl: './support-types-modal.component.scss',
})
export class SupportTypesModalComponent implements AfterViewInit {
  private readonly fb = inject(FormBuilder);
  private readonly modal = inject(SklModalService);
  private readonly destroyRef = inject(DestroyRef);

  readonly data = input<ISupportTypeModalData | null>(null);

  protected readonly isEdit = computed(() => !!this.data()?.id);

  protected readonly helper = 'configuration.support-types.translation';
  ...
  protected onCancel(): void {
    this.modal.close<ISupportTypeModalResult>({
      saved: false,
  ...
  private buildPayload(): ISupportTypeModalResult['payload'] {
